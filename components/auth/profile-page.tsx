...
export function ProfilePage() {
  ...
  const saveOrganization = async (event: React.FormEvent) => { event.preventDefault(); try { await authApi.updateOrganization(organization); await refreshProfile(); toast.success("Đã lưu thông tin CLB."); } catch { toast.error("Không thể lưu thông tin CLB."); } };
  ...
  if (!user) return null;
  const field = (key: keyof OrganizationProfile, label: string) => <div className="grid gap-2"><Label>{label}</Label><Input value={organization[key] ?? ""} onChange={(e) => setOrganization({ ...organization, [key]: e.target.value })} /></div>;
